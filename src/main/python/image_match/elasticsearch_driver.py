...
class SignatureES(SignatureDatabaseBase):
    ...
    def search_single_record(self, rec):
        ...
        for i, row in enumerate(formatted_res):
            row["dist"] = dists[i]
        formatted_res = filter(
            lambda y: y["dist"] < self.distance_cutoff, formatted_res
        )
    ...
    def insert_single_record(self, rec, refresh_after=False):
        ...
        self.es.index(
            index=self.index, doc_type=self.doc_type, body=rec, refresh=refresh_after
        )
    ...
    def delete_duplicates(self, path):
        ...
        matching_paths = [
            item["_id"]
            for item in self.es.search(
                body={"query": {"match": {"path": path}}}, index=self.index
            )["hits"]["hits"]
